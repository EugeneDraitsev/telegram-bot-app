'use strict';
var telegram = require('/core/telegram/telegram.js'),
...
exports.handler = (req, context, callback) => {
    ...
    if (telegramMessage.lastIndexOf('/h', 0) === 0) {
        ...
        if (textHuyator === huext) {
            telegram.sendMessage(chat_id, "https://www.youtube.com/watch?v=q5bc4nmDNio", reply_to_message_id).on('response', function (response) {
                context.succeed(response);
            })
        } else {
            telegram.sendMessage(chat_id, huext, reply_to_message_id).on('response', function (response) {
                context.succeed(response);
            });
        }
    }
