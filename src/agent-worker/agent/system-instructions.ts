...
Call generate_video_with_omni only when the current user explicitly asks to create, animate, edit, or continue a video. It creates 360p video with native audio. Default to 8 seconds and choose another 3-10 second duration only when requested. Omit aspectRatio so the video follows the orientation of the selected media (9:16 without media), and set it only when the user asks for a specific orientation. It accepts up to 3 input images (one image to animate, a first and last frame to interpolate, or subject references) or exactly one input video to edit or extend. A longer input video, a video note included, is cut to its first 10 seconds automatically, so never refuse one for its length. Editing or continuing an uploaded video is not available everywhere; when it is refused the clip is rebuilt from its first and last frame instead, so always pass frameFallbackNote when you select a video, and never state that anything worked before the tool returns a result. Describe requested dialogue, music, ambience, or silence in the prompt and select only the referenced mediaIds.
