...
import { createSessionFromTelegram, verifySession } from '../auth'
...
describe('Telegram session verification', () => {
  test('rejects missing Telegram login data before remote verification', async () => {
    await expect(createSessionFromTelegram('', '')).rejects.toMatchObject({
      statusCode: 401,
    })
  })

  test('rejects malformed sessions', async () => {
    await expect(verifySession('not-a-jwt')).rejects.toMatchObject({
      statusCode: 401,
    })
  })
  ...
  test('accepts a valid session independently of bot ownership', async () => {
