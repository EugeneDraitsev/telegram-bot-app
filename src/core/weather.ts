...
const getWeatherUrlForecast = (locationURL: string): string =>
  `http://api.openweathermap.org/data/2.5/forecast/daily?q=${encodeURI(locationURL)}&units=metric&lang=ru&APPID=${weatherToken}`

const getWeatherUrlNow = (locationURL: string): string =>
  `http://api.openweathermap.org/data/2.5/weather?q=${encodeURI(locationURL)}&units=metric&lang=ru&APPID=${weatherToken}`
