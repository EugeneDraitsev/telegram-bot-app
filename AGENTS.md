...
- `src/agent-worker/agent/models.ts` is the source of truth for every model,
  its reasoning effort, its fallback and its timeout. Do not duplicate model
  names here or hardcode them anywhere else.
